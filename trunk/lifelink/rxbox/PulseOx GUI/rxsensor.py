    ...
    def run(self):
        ...
        while True:
            ...
            while 1:
                ...
                spo2_val=0
                while (time.time() - BaseTime <= interval):
                               ...
                               (s[len(s)-1]== chr(int('0xa8',0))):              #if start flag is detected and last char is end flag
                                if ((i+7) <= (len(s)-1)):                       #if indeces of succeeding items to be accessed are still possible
                                    ...
                                    if (ord(s[index+1]) <= 100):                #if signal quality is possible ( less than/equal to 100% ), transfer the readings
                                        ...
                                        print "spo2:", spo2_val
                                        wx.CallAfter(self.ParentWindow.UpdateSpO2Display, str(spo2_val))
                                        ...
                                        wx.CallAfter(self.ParentWindow.UpdateSpO2Display, str(prev_spo2_val))
...
class ECG:
    ...
    def __init__(self, port=1,baud=230400,timeout=None):
        self.port = port                #set port number
        self.baudrate = baud            #set baud rate
        self.timeout = timeout          #set timeout
    ...
    def get(self):
        ...
        myFile = open(self.ecg_testfile, 'r')   #open testfile containing pseaudo sp02 values
        value = myFile.readline()               #read each line(sample) terminated by \n
        BaseTime = time.time()                  #get reference time
        count = 0                               #counts the number of edf file produced
        while True:
            ecg =[]                             #initialize list for sp02 values
            if value == '':                     #when EOF is encountered...finished parsing text file
                print 'break'
                break
            while (time.time() - BaseTime <= interval):
                if value == '':
                ...
                sample = int(value)
                ecg.append(sample)
                # to simulate 500Hz sampling frequency, get samples every 0.002 sec
                time.sleep(0.002)
                value = myFile.readline()
            Biosignal_ECG = BioSignal('ECG', 'None', 'mV', -32767, 32767, -32767, 32767, 'None', 500, ecg) 
            return Biosignal_ECG
        return Biosignal_ECG
...
class PulseRate(threading.Thread):
    ...
    def run(self):
        ...
        myFile = open(self.bpm_testfile, 'r')   #open testfile containing pseaudo sp02 values
        ...
        while True:
            ...
            while True:
                ...
                bpm =[]                        #initialize list for sp02 values
                if value == '':                 #when EOF is encountered...finished parsing text file
                    print 'break'
                    break
                while (time.time() - BaseTime <= interval):
                    if value == '':
                        print 'break'
                        break
                    #wx.CallAfter(self.ParentWindow.UpdateBPMDisplay, value)
                    sample = int(value)
                    bpm.append(sample)
                # to simulate 10Hz sampling frequency, get samples every 0.1 sec
                    time.sleep(0.1)
                    value = self.myFile.readline()
                BaseTime = time.time()
                Biosignal_BPM = BioSignal('Pulse Rate', 'None', 'BPM', 50, 100, 50, 100, 'None', 10, bpm) 
                
                return (self.ParentWindow.BioSignals.append(Biosignal_BPM))

            
            return (self.ParentWindow.BioSignals.append(Biosignal_BPM))
            wx.CallAfter(self.ParentWindow.onClickStart,wx.EVT_BUTTON)
