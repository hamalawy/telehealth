...
class VideoconfServThr(threading.Thread):
    def __init__(self, threadNum, window):
        ...
        self.vlc_run = './vlc v4l:// :v4l-vdev="/dev/video0" :v4l-adev="/dev/dsp" :v4l-channel=1 :v4l-norm=pal :v4l-size=320x240 --sout \'#duplicate{dst=display,dst="transcode{vcodec=DIV3,acodec=mpga,vb=96,ab=16,deinterlace}:standard{access=udp,mux=ts,dst=' + self.host + ':' + self.port + '}"}\''
