...
class ECGorig:
    ...
    def __init__(self, parent, port=22, baud=230400, timeout=None):
    ...
    def get_ecm(self):
        ...
        while (contact_counter < 3) and (time.time()<(basetime+15)):
            ...
            #print "RAW ecgmodule reply for ECM:", raw_packet
            packet = self.byte_destuff(raw_packet)              # decodes reply packet for byte-stuffed data; packet here is a list
            #print "PRE-PROCESSED ecgmodule reply for ECM in HEX:", packet
    ...
    def inquiry(self):
        ...
        #print "protocol version inquiry reply:", packet
    ...
    def get_ecg(self):
        ...
        # if number of samples < required, pad with zeroes
...
class SPO2orig:
    ...
    def __init__(self, parent, port=1,baud=9600,timeout=None):
        ...
        try:
            self.spo2 = serial.Serial(self.port, baudrate=self.baudrate, timeout=self.timeout, xonxoff=0)      
        except serial.SerialException:
            print "Unable to open COM port",self.port+1, "\nPlease check serial port settings."
            #return 0
    ...
    def byte_destuff(self, raw_packet):
        ...
        for index in range(1, (len(raw_packet)-1)):             # end flag excluded (next last character is the last candidate for destuffing)
            if skip_counter == 1:                               # re-initialize skip_counter
                ...
                if reply_packet[index] == chr(0xa9):
                    destuffed.append(chr( ord(reply_packet[index+1])|int(0x20) ))       # append next char(OR-linked with 0x20) to 'destuffed' data_stream list
                    skip_counter+= 1                                                    # move to next next char
                    continue                                                            # skip
                else:
                    destuffed.append(chr( ord(reply_packet[index]) ))                   # char is not 0xa9, simply append it to 'destuffed' data_stream list        
                    continue
    ...
    def reply_parser(self, destuffed_packet):
        ...
        CHIPOX_channel = ord(destuffed_packet[1])
        if CHIPOX_channel == 127:
            sig_quality = ord(destuffed_packet[6])   
            print "signal quality:", sig_quality, "%"
            if (signal_quality <= 100):                             # if CHIPOX channel is ok and signal quality is not erroneous
                spo2 = ord(destuffed_packet[3])
                bpm = (ord(destuffed_packet[4])<<8) + ord(destuffed_packet[5])
                sig_amp = ord(destuffed_packet[7])
                print "spo2 reading:", spo2
                print "pulse_rate:", pulse_rate, "bpm"
                ...
                if len(self.spo2_values)>0:                                # if self.spo2 list already contains something
                    ...
                    prev_pulse_rate = self.pulse_rate[len(self.pulse_rate)-1]
        ...
        wx.CallAfter(self.parentPanel.updateSPO2Display, str(spo2))
        wx.CallAfter(self.parentPanel.updateBPMDisplay, str(bpm))
    ...
    def get_spo2(self):
        ...
        Biosignal_SPO2 = BioSignal('SpO2 finger', 'IR-Red sensor',    '%',      0,    100,    0,    100,    'None',     150,  self.spo2_values)
        ##########################       ^               ^             ^        ^      ^      ^      ^        ^          ^            ^
        ##########################     label        sensor-type     physical   phys.  phys.  digi.  digi     pre-     Nsamples       list
        ##########################                                 dimension   min    max    min    max    filtering

        Biosignal_BPM = BioSignal('SpO2 finger', 'IR-Red sensor',   'bpm',     0,    300,    0,    300,    'None',    150,   self.pulserate_values)
        #########################        ^               ^             ^        ^      ^      ^     ^        ^         ^               ^
        #########################     label        sensor-type     physical   phys.  phys.  digi.  digi     pre-    Nsamples         list
        #########################                                 dimension   min    max    min    max    filtering
    ...
    def reset(self):
        """reset serial port input buffer"""
        self.spo2.flushInput()           #data currently stored in the input buffer is flushed and operation continues
...
class SPO2:
    ...
    def get(self):
        ...
        try:
            value = self.parentPanel.RawBioSignals[1][self.index]
            BaseTime = time.time()              #get reference time
            ...
            while (len(bpm) != 150):
                ...
                CallAfter(self.parentPanel.updateSPO2Display, str(value))
                    #sample = int(value)
                    #bpm.append(sample)
...
class PulseRate:
    ...
    def get(self):
        ...
        try:
            value = self.parentPanel.RawBioSignals[2][self.index]
            BaseTime = time.time()              #get reference time
            count = 0                           #counts the number of edf file produced
            bpm = []
            ...
            while (len(bpm) != 150):
                ...
                CallAfter(self.parentPanel.updateBPMDisplay, str(value))
                    #sample = int(value)
                    #bpm.append(sample)
                    # to simulate 10Hz sampling frequency, get samples every 0.1 sec
                self.index += 1
...
class ECG:
    ...
    def __init__(self, parent):
        ...
        #self.bpm_testfile = "C:/Users/ARLAN ROIE SANTOS/Des#ktop/bpm_testfile2.txt"
        #self.myFile = open(self.bpm_testfile, 'r')   #open testfile containing pseaudo sp02 values
        #self.ParentWindow = ParentWindow
    ...
    def get(self):
        ...
        try:
            #self.ecg_leadII.append(self.parentPanel.RawBioSignals[0][self.index] * self.gain)
            #BaseTime = time.time()              #get reference time

            #count = 0                           #counts the number of edf file produced
            

            while (len(self.ecg_leadII) != 1500):
                self.index += 1
                self.ecg_leadII.append(self.parentPanel.RawBioSignals[0][self.index] * self.gain)
                #CallAfter(self.parentPanel.updateBPMDisplay, str(value))
                    #sample = int(value)
                ...
                #time.sleep(0.01)
                try:
                    value = self.parentPanel.RawBioSignals[0][self.index]
                except IndexError:
                    break
