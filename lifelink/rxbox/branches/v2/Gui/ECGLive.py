...
class ECGLive:
    ...
    def ecg_lead(self):
        ...
        if len(self.leadV6_temp)>1:
            ...
            del self.leadII_temp[:]
            del self.leadIII_temp[:]
            del self.leadV1_temp[:]
            del self.leadV2_temp[:]
            del self.leadV3_temp[:]
            del self.leadV4_temp[:]
            del self.leadV5_temp[:]
            del self.leadV6_temp[:]
##            self.ecg_leadII = besselfilter(self.leadII_temp)
##            self.ecg_leadIII = besselfilter(self.leadIII_temp)
##            self.ecg_leadV1 = besselfilter(self.leadV1_temp)
##            self.ecg_leadV2 = besselfilter(self.leadV2_temp)
##            self.ecg_leadV3 = besselfilter(self.leadV3_temp)
##            self.ecg_lead4 = besselfilter(self.leadV4_temp)
##            self.ecg_lead5 = besselfilter(self.leadV5_temp)
##            self.ecg_lead6 = besselfilter(self.leadV6_temp)
    ...
    def get_ecm(self, count=3, check = True):
        ...
        basetime = time.time()
        while (contact_counter < count) and (time.time()<(basetime+15)):
        ...
        if ecm_endtime >(basetime+15) or (not check and not self.electrode_check()):
            self.ecm_status = 0
        else:
            self.ecm_status = 1
    ...
    def electrode_stat(self):
        """Quick check of ECG lead status"""
        self.start_ecm()
    ...
    def stop(self):
        ...
        if self.ecm_status == 1 and self.start_flag == 1:   
            self.stop_ecg()
            self.ecg.close()
    ...
    def Pop(self,start=0,end=0):
        ...
        del self.ecg_leadI[start:end]
        list = self.ecg_leadII[start:end]        
        del self.ecg_leadII[start:end]                # "dynamic" list that continuously accommodates lead II readings
        del self.ecg_leadIII[start:end]               # "dynamic" list that continuously accommodates lead III readings
        del self.ecg_leadV1[start:end]
        del self.ecg_leadV2[start:end]
        del self.ecg_leadV3[start:end]
        del self.ecg_leadV4[start:end]
        del self.ecg_leadV5[start:end]
        del self.ecg_leadV6[start:end]
        del self.ecg_leadaVR[start:end]
        del self.ecg_leadaVL[start:end]
        del self.ecg_leadaVF[start:end]
        return list
...
class ECGThread:
    ...
    def Stop_Thread(self):
        ...
        print 'Stopping ECG Thread'
        if self.alive:
            self.alive = False
            time.sleep(self.daqdur)
        self.ECG.stop()
