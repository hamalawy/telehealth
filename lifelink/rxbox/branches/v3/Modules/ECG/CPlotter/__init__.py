"""
Project LifeLink: c-sdl plotter

Authors:    Sy, Luke Wicent
            Luis Sison, PhD
            ------------------------------------------------
...
class CPlotter:
    def __init__(self, panel=None, mode='normal', time=3, cont=True,data=False):
        self.panel = panel
        self.mode = mode
        self.time = time
        self.cont = cont
    ...
    def Open(self):
        ...
        if not self.on:
            ...
            self.comm.stdin.write("%d,%d,%d,%d,Modules/ECG/CPlotter/%s\n"%(CPlotterMode[self.mode][0],\
